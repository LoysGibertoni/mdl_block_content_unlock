...
class block_game_content_unlock_edit_form extends block_edit_form
{
    ...
    protected function specific_definition($mform)
	{
		...
		if(has_capability('block/game_content_unlock:addunlocksystem', $context))
		{
			...
			$html = '<table><tr><th>ID</th><th>Condições</th><th>Módulo</th><th>Descrição</th><th>Editar</th><th>Remover</th></tr>';
			foreach($unlock_systems as $value)
			{
				...
				$html = $html . '<tr><td>' . $value->id . '</td><td>' . $eventsarray[$value->conditions] . '</td><td>' . $cm->name . '</td><td>' . $value->eventdescription . '</td><td>' . html_writer::link($urledit, 'Editar') . '</td><td>' . html_writer::link($urlremove, 'Remover') . '</td></tr>';
			}
